...
La feature **MyProfile** centralise toute la logique métier et technique liée aux my_profile(s) dans le projet.
...
* **MyProfile_README.md** → Documentation de la feature.
...
* **Découplage total** : Le MyProfile`Controller` ne dépend pas directement d’une techno (ex: Supabase). Il utilise uniquement les **usecases** définis au niveau du domaine.
...
# ⚙️ MyProfile Use Cases

Les **use cases** définissent les actions métiers disponibles pour la gestion des my_profile(s).
...
* Toujours injecter un **repository conforme à `MyProfileRepository`** dans le constructeur.
...
* Manipuler uniquement des **`EntityMyProfile`** (jamais de modèles `data/models`) dans le domaine.
