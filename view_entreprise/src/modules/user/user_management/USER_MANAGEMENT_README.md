...
La feature **UserManagement** centralise toute la logique métier et technique liée aux user_management(s) dans le projet.
...
* **UserManagement_README.md** → Documentation de la feature.
...
* **Découplage total** : Le UserManagement`Controller` ne dépend pas directement d’une techno (ex: Supabase). Il utilise uniquement les **usecases** définis au niveau du domaine.
...
# ⚙️ UserManagement Use Cases

Les **use cases** définissent les actions métiers disponibles pour la gestion des user_management(s).
...
* Toujours injecter un **repository conforme à `UserManagementRepository`** dans le constructeur.
...
* Manipuler uniquement des **`EntityUserManagement`** (jamais de modèles `data/models`) dans le domaine.
