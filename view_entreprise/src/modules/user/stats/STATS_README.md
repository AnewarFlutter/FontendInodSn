...
La feature **Stats** centralise toute la logique métier et technique liée aux stats(s) dans le projet.
...
* **Stats_README.md** → Documentation de la feature.
...
* **Découplage total** : Le Stats`Controller` ne dépend pas directement d’une techno (ex: Supabase). Il utilise uniquement les **usecases** définis au niveau du domaine.
...
Les **use cases** définissent les actions métiers disponibles pour la gestion des stats(s).
...
* Toujours injecter un **repository conforme à `StatsRepository`** dans le constructeur.
...
* Manipuler uniquement des **`EntityStats`** (jamais de modèles `data/models`) dans le domaine.
