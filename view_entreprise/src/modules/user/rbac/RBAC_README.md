...
La feature **Rbac** centralise toute la logique métier et technique liée aux rbac(s) dans le projet.
...
* **Rbac_README.md** → Documentation de la feature.
...
* **Découplage total** : Le Rbac`Controller` ne dépend pas directement d’une techno (ex: Supabase). Il utilise uniquement les **usecases** définis au niveau du domaine.
...
Les **use cases** définissent les actions métiers disponibles pour la gestion des rbac(s).
...
* Toujours injecter un **repository conforme à `RbacRepository`** dans le constructeur.
...
* Manipuler uniquement des **`EntityRbac`** (jamais de modèles `data/models`) dans le domaine.
