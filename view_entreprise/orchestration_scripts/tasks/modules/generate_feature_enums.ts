 ...
 */
export function generateFeatureEnums(
    module: ModuleSchema,
    f: FeatureSchema,
    project?: Project
) {
    ...
    const enums = f.$enums ?? [];
    const enumsDir = path.join(featurePath, "domain", "enums");
    ...
    enums
        ...
        .forEach((e) => {
            ...
            const content = T.enumBlock(f.$name, e) + "\n";
    ...
    const mergedFileName = `${getFileName(casing, `${featureFile}_enums`)}.ts`;
    ...
    merged.forEach((e) => {
        newEnumBlocks.set(e.$name, T.enumBlock(f.$name, e));
    });
