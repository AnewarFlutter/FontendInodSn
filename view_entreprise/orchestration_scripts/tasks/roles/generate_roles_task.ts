...
const ROLES_ENUM_PATH = path.join(process.cwd(), "src", "shared", "enums", "user_role.ts");
const ROLES_CONST_PATH = path.join(process.cwd(), "src", "shared", "constants", "values", "role_permissions.ts");

export function generateRolesTask(config: ProjectConfigSchema, project?: Project) {
    ...
    for (const role of sorted) {
        for (const perm of role.$permissions || []) {
            ...
            if (!permissionKeys.has(perm)) {
                throw new Error(
                    `Rôle ${role.$key} référence une permission inexistante : ${perm}\n` +
