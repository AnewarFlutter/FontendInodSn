...
export function getDefaultValueForType(
    type: string,
    enumValues: Map<string, string[]> = new Map()
): string {
    // Nettoyer le type
...
export const placeholderMethods = `// Méthodes générées dynamiquement`;

function loremIpsum(options: { count: number; units: "words" | "sentences" | "paragraphs" }): string {
    if (options.units === "words") return "Sunt ex magna sint consequat est."
    else if (options.units === "sentences") return "Cupidatat consectetur laboris qui reprehenderit irure magna ut quis nisi id officia pariatur sit ex."
    else {
        if(options.count === 1) 
...
export const MiniLoremValue = loremIpsum({ count: 1, units: 'paragraphs' });
export const MediumLoremValue = loremIpsum({ count: 2, units: 'paragraphs' });
export const ExtraLoremValue = loremIpsum({ count: 5, units: 'paragraphs' });