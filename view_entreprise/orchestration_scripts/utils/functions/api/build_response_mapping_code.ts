...
import { UsecaseSchema, FeatureSchema, ModulesConfigSchema } from "../../../config_validator";
 ...
 */
export function buildResponseMappingCode(
    usecase: UsecaseSchema,
    feature: FeatureSchema,
    route: ApiRoute,
    config: ModulesConfigSchema,
    endpointKey: string
): { returnCode: string; imports: string[]; dataType: string } {
    const returnType = usecase.$returnType;
    if (!returnType) return { returnCode: "data as unknown;", imports: [], dataType: "Record<string, unknown>" };

    const isLocalEntity = returnType.$type === `Entity${toCasing.pascalCase(feature.$name)}`;
    const isList = returnType.$list === true;
    const isNullable = returnType.$nullable === true;
    const modelName = `Model${toCasing.pascalCase(feature.$name)}`;
    ...
    if (isLocalEntity) {
        const modelImport = `../models/${getFileName(casing, `model_${featureFile}`)}`;
        ...
        if (responseSchema && typeof responseSchema === "object") {
            const arrayField = Object.entries(responseSchema).find(
                ([, v]) => typeof v === "object" && v.$type === "array"
            )?.[0];
            const objectField = Object.entries(responseSchema).find(
                ([, v]) => typeof v === "object" && v.$type === "object"
            )?.[0];
