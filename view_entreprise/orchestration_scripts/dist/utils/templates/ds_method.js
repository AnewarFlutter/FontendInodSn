...
exports.DataSourceMethodTemplate = void 0;
const casing_1 = require("../casing");
const DataSourceMethodTemplate = (uc, feature) => {
    ...
    const paramType = (param) => {
        ...
        if (type === `Entity${casing_1.toCasing.pascalCase(feature.$name)}`)
            type = `Model${casing_1.toCasing.pascalCase(feature.$name)}`;
        if (type === `Model${casing_1.toCasing.pascalCase(feature.$name)}`)
            type = `Model${casing_1.toCasing.pascalCase(feature.$name)}`;
        if (type === `Partial<Entity${casing_1.toCasing.pascalCase(feature.$name)}>`)
            type = `Partial<Model${casing_1.toCasing.pascalCase(feature.$name)}>`;
        if (type === `Partial<Model${casing_1.toCasing.pascalCase(feature.$name)}>`)
            type = `Partial<Model${casing_1.toCasing.pascalCase(feature.$name)}>`;
        return type;
    };
...
exports.DataSourceMethodTemplate = DataSourceMethodTemplate;
