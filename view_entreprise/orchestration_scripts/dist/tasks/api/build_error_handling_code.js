...
exports.buildErrorHandlingCode = buildErrorHandlingCode;
 ...
 */
function buildErrorHandlingCode(route, returnTypeNullable, endpointKey) {
    const byStatus = route.$response?.$byStatus ?? {};
    const lines = ["    if (error) {", "      switch (status) {"];
    const errorTypes = [];
